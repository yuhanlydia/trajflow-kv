...
def generation_signature(engine,kind,donor_limit,normalization):
    return digest(dict(kind=kind,donor_limit=donor_limit,normalization=normalization,
        ...
        four_bit=engine.four_bit,protocol_version=1))
...
def label_record(engine,row,kind,donor_limit=3,normalization='mean'):
    ...
    for j,span in enumerate(context.spans):
        ...
        donors=[d for d in eligible if d!=j and 0<=d<len(context.spans)
                and context.spans[d][1]-context.spans[d][0]==span[1]-span[0]][:donor_limit]
...
def main():
    ...
    p.add_argument('--kind',choices=['local_action_logprob','candidate_value_proxy'],default='candidate_value_proxy')
    ...
    for row in rows:
        if row['prefix_id'] in byid:
            ...
            if prior.get('teacher_context_hash')!=digest(public_record(row)) or prior.get('teacher_source_hash')!=digest(row) or prior.get('generation_signature')!=generation_signature(e,a.kind,a.donors,a.normalization):
                raise ValueError('Resume source/protocol/model mismatch; use a new output path')
            continue
        done.append(label_record(e,row,a.kind,a.donors,a.normalization));write_jsonl(a.output,done)
